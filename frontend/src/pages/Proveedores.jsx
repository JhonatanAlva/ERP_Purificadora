...
import {
  ...
  Phone, Mail, MapPin, CheckCircle, XCircle, X, Package
} from "lucide-react";
...
export default function Proveedores() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setProductosInventario(prodData.filter(p => p.activo === true || p.activo === "true"));
    } catch {
  ...
  const loadProductos = async () => {
    try {
      ...
      setProductosInventario(data.filter(p => p.activo === true || p.activo === "true"));
    } catch { /* silencioso */ }
  };
  ...
  const handleSaveProd = async () => {
    ...
    try {
      ...
      const activos = prodData.filter(p => p.activo === true || p.activo === "true");
  ...
  const openEdit = (p) => {
    ...
    const ids = p.productos_suministra
      ? p.productos_suministra.split(",").map(s => s.trim()).filter(Boolean)
      : [];
    setSelectedIds(ids);
    setEditId(p.id);
    setError("");
  ...
  const setProd = (field) => (e) => setFormProd({ ...formProd, [field]: e.target.value });

  const getNombresProductos = (productos_suministra) => {
    if (!productos_suministra) return null;
    const ids = productos_suministra.split(",").map(s => s.trim()).filter(Boolean);
    return ids
  ...
  const inputCls = "w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-300";
      ...
      <div className="space-y-5">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map(p => {
              const nombresProductos = getNombresProductos(p.productos_suministra);
              return (
                <div key={p.id}
                  ...
                  <div className="space-y-1.5 text-sm text-gray-600">
                    ...
                    {nombresProductos && (
                      <div className="mt-2">
                        <p className="text-xs text-gray-400 mb-1">Productos del inventario:</p>
                        <p className="text-xs bg-sky-50 text-sky-700 px-2 py-1 rounded-lg">{nombresProductos}</p>
                      </div>
                    )}
                    {p.notas && (
                      <div className="mt-1">
                        <p className="text-xs text-gray-400 mb-1">Otros productos:</p>
                        <p className="text-xs bg-gray-50 text-gray-600 px-2 py-1 rounded-lg">{p.notas}</p>
                      </div>
                    )}
                  </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <input value={form.nombre} onChange={set("nombre")} placeholder="Nombre del proveedor" className={inputCls} />
                </div>
                ...
                <div>
                  ...
                  <input value={form.telefono} onChange={set("telefono")} placeholder="555-123-4567" className={inputCls} />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                  <input type="email" value={form.email} onChange={set("email")} placeholder="correo@proveedor.com" className={inputCls} />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Dirección</label>
                  <input value={form.direccion} onChange={set("direccion")} placeholder="Dirección" className={inputCls} />
                </div>
                ...
                <div className="sm:col-span-2">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                      onClick={() => { setFormProd(EMPTY_PROD); setErrorProd(""); setModalProd(true); }}
                  ...
                  <p className="text-xs text-gray-400 mb-2">
                    Estos productos aparecerán al crear una compra con este proveedor
                  </p>
                  {productosInventario.length === 0 ? (
                    <p className="text-sm text-gray-400 italic">No hay productos en inventario</p>
                  ) : (
                ...
                <div className="sm:col-span-2 flex items-center gap-3">
                  ...
                  <label htmlFor="activo_prov" className="text-sm font-medium text-gray-700">Proveedor activo</label>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-60 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button onClick={() => setModalProd(false)} className="p-2 hover:bg-gray-100 rounded-lg">
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>

            <div className="p-6 space-y-4">
              {errorProd && <p className="text-sm text-red-500 bg-red-50 px-3 py-2 rounded-lg">{errorProd}</p>}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={formProd.tipo} onChange={setProd("tipo")}
                    className="w-full mt-1 bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition">
                ...
                <div>
                  ...
                  <input value={formProd.unidad} onChange={setProd("unidad")}
                    className="w-full mt-1 bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={formProd.precio_venta} onChange={setProd("precio_venta")}
                    className="w-full mt-1 bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Precio Costo</label>
                  <input type="number" value={formProd.precio_costo} onChange={setProd("precio_costo")}
                    className="w-full mt-1 bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Stock Inicial</label>
                  <input type="number" value={formProd.stock_actual} onChange={setProd("stock_actual")}
                    className="w-full mt-1 bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Stock Mínimo</label>
                  <input type="number" value={formProd.stock_minimo} onChange={setProd("stock_minimo")}
                    className="w-full mt-1 bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition" />
                </div>
              </div>
              ...
              <div>
                ...
                <input placeholder="Descripción opcional" value={formProd.descripcion} onChange={setProd("descripcion")}
                  className="w-full mt-1 bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-sm transition" />
              </div>
            </div>
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-gray-100">
              ...
              <button onClick={handleSaveProd} disabled={savingProd || !formProd.nombre.trim()}
