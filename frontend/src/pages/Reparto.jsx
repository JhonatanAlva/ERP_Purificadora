...
import {
  Truck, Plus, Search, Eye, X, CheckCircle,
  ChevronLeft, ChevronRight, Loader2, AlertCircle,
  MapPin, Clock, Package, DollarSign, AlertTriangle,
...
const Reparto = () => {
  ...
  const cargarCatalogos = async () => {
    ...
    if (resProd.status === "fulfilled") setProductos(toArray(resProd.value.data));
  ...
  useEffect(() => { setPageLocal(1); }, [search, filtroEstado]);
  ...
  const productosDisponibles = productos.filter((p) => (p.stock_actual || 0) > 0);
  ...
  const diferenciaEfectivo = (parseFloat(formRegreso.efectivo_entregado) || 0) - efectivoEsperado;
    ...
    <Layout>
      <div className="space-y-5 p-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button
            onClick={() => { setModalSalida(true); setError(""); }}
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Rutas hoy", value: stats.garrafones_hoy + " grf", color: "text-sky-600" },
              { label: "En ruta", value: stats.en_ruta, color: "text-blue-600" },
              { label: "Efectivo total", value: fmt(stats.efectivo_total), color: "text-emerald-600" },
              { label: "Total garrafones", value: stats.total_garrafones, color: "text-gray-800" },
            ].map((s) => (
          ...
          <div className="flex gap-2 flex-wrap">
            {["todos", "en_ruta", "cerrada", "pendiente"].map((e) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filtroEstado === e
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {tieneDif && <AlertTriangle className="w-4 h-4 text-amber-500" />}
                      </div>
                      <p className="text-xs text-gray-400 font-mono">{r.folio}</p>
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${ESTADO_STYLES[r.estado] || "bg-gray-100 text-gray-600"}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 mb-3">
                    ...
                    <div className="text-gray-500">
                      Fecha: <span className="font-medium text-gray-700">{r.fecha}</span>
                    </div>
                    <div className="flex items-center gap-1 text-gray-500">
                      <Clock className="w-3 h-3 text-gray-400" />
                      Salida: <span className="font-medium text-gray-700 ml-1">{r.hora_salida || "—"}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3 text-sm mb-3">
                    <div className="flex-1 bg-sky-50 rounded-lg p-2 text-center">
                      ...
                      <p className="font-bold text-sky-700 text-lg">{r.total_garrafones_salida || 0}</p>
                    </div>
                    {r.estado === "cerrada" && (
                      <div className="flex-1 bg-emerald-50 rounded-lg p-2 text-center">
                        <p className="text-xs text-gray-500">Efectivo</p>
                        <p className="font-bold text-emerald-700 text-sm">{fmt(r.efectivo_entregado)}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {r.estado === "cerrada" && (
                    <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                      <div className={`px-2 py-1.5 rounded-lg ${Math.abs(r.diferencia_efectivo || 0) > 0.01 ? "bg-red-50 text-red-700" : "bg-emerald-50 text-emerald-700"}`}>
                        <span className="font-medium">Dif. Efectivo: </span>{fmt(r.diferencia_efectivo)}
                      </div>
                      <div className={`px-2 py-1.5 rounded-lg ${Math.abs(r.diferencia_garrafones || 0) > 0 ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-700"}`}>
                        <span className="font-medium">Dif. Garrafones: </span>{r.diferencia_garrafones || 0}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
                    ...
                    {r.estado === "en_ruta" && (
                      ...
                      >
                        <CheckCircle className="w-3.5 h-3.5" /> Registrar Regreso
                      </button>
                    )}
                  </div>
                </div>
        ...
        {totalPaginasLocal > 1 && (
          <div className="flex items-center justify-between bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-3 text-sm text-gray-500">
            <span>{rutasFiltradas.length} rutas · Página {pageLocal} de {totalPaginasLocal}</span>
            <div className="flex gap-1">
              <button onClick={() => setPageLocal((p) => Math.max(1, p - 1))} disabled={pageLocal === 1}
                className="p-1.5 rounded-lg hover:bg-gray-100 disabled:opacity-30 transition-colors">
                <ChevronLeft className="w-4 h-4" />
              </button>
              {Array.from({ length: totalPaginasLocal }, (_, i) => i + 1).map((n) => (
              ...
              <button onClick={() => setPageLocal((p) => Math.min(totalPaginasLocal, p + 1))} disabled={pageLocal === totalPaginasLocal}
                className="p-1.5 rounded-lg hover:bg-gray-100 disabled:opacity-30 transition-colors">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
              <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
                <ArrowDownCircle className="w-5 h-5 text-sky-500" /> Registrar Salida
              </h3>
              <button onClick={() => setModalSalida(false)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="time" value={formSalida.hora_salida}
                    onChange={(e) => setFormSalida({ ...formSalida, hora_salida: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-300"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Piloto *</label>
                  <input value={formSalida.piloto}
                    onChange={(e) => setFormSalida({ ...formSalida, piloto: e.target.value })}
                    placeholder="Nombre del piloto"
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-300"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Camión / Unidad</label>
                  <input value={formSalida.camion}
                    onChange={(e) => setFormSalida({ ...formSalida, camion: e.target.value })}
                    placeholder="Placa o identificación"
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-300"
                  />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Zona / Ruta</label>
                  <input value={formSalida.zona_ruta}
                    onChange={(e) => setFormSalida({ ...formSalida, zona_ruta: e.target.value })}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2 flex items-center gap-1">
                  <Package className="w-4 h-4 text-gray-400" /> Productos a cargar
                </label>
                {productosDisponibles.length === 0 ? (
                  <p className="text-sm text-gray-400 italic">No hay productos disponibles</p>
                ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-36 overflow-y-auto border border-gray-100 rounded-xl p-2">
                    {productosDisponibles.map((p) => (
                      <button key={p.id} onClick={() => agregarProductoSalida(p)}
              ...
              {formSalida.detalle.length > 0 && (
                <div className="border border-gray-100 rounded-xl overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="text-center px-3 py-2 text-xs font-semibold text-gray-500">Cantidad</th>
                      ...
                      {formSalida.detalle.map((item) => (
                        <tr key={item.producto_id}>
                          ...
                          <td className="px-3 py-2 text-center">
                            <input type="number" min="1" value={item.cantidad_salida}
                              onChange={(e) => actualizarCantidadSalida(item.producto_id, e.target.value)}
                          ...
                          <td className="px-3 py-2 text-center">
                            <button onClick={() => quitarProductoSalida(item.producto_id)}
                  ...
                  <div className="flex justify-between items-center font-bold text-sm px-3 py-2.5 bg-sky-50 border-t border-sky-100">
                    ...
                    <span className="text-sky-700 text-base">{totalGarrafonesSalida}</span>
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <textarea value={formSalida.observaciones}
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-gray-100 shrink-0">
              ...
              <button onClick={handleGuardarSalida}
                disabled={saving || !formSalida.piloto || formSalida.detalle.length === 0}
                ...
                {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Guardando...</> : "Registrar Salida"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
              ...
              <button onClick={() => setModalRegreso(null)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-5">
              ...
              <div className="bg-sky-50 border border-sky-100 rounded-xl p-4">
                <p className="text-xs font-semibold text-sky-700 uppercase mb-2">Resumen de salida</p>
                <div className="grid grid-cols-3 gap-3 text-center text-sm">
                  <div>
                    <p className="text-xs text-gray-500">Folio</p>
                    <p className="font-mono font-bold text-gray-700">{modalRegreso.folio}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Garrafones salida</p>
                    <p className="font-bold text-sky-700 text-xl">{modalRegreso.total_garrafones_salida || 0}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500">Zona</p>
                    <p className="font-bold text-gray-700">{modalRegreso.zona_ruta || "—"}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="time" value={formRegreso.hora_regreso}
                    onChange={(e) => setFormRegreso({ ...formRegreso, hora_regreso: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-300"
                  />
                </div>
              </div>
              ...
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-emerald-700">{fmt(efectivoEsperado)}</span>
                </div>
                <div>
                  ...
                  <input type="number" min="0" step="0.01"
                    ...
                    onChange={(e) => setFormRegreso({ ...formRegreso, efectivo_entregado: e.target.value })}
                    placeholder="0.00"
                ...
                {formRegreso.efectivo_entregado !== "" && (
                  <div className={`flex justify-between items-center font-bold text-sm p-2 rounded-lg ${diferenciaEfectivo < -0.01 ? "bg-red-100 text-red-700" :
                    diferenciaEfectivo > 0.01 ? "bg-amber-100 text-amber-700" :
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Notas del regreso</label>
                <textarea value={formRegreso.notas_regreso}
                  onChange={(e) => setFormRegreso({ ...formRegreso, notas_regreso: e.target.value })}
                  rows={2} placeholder="Observaciones del piloto..."
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-sky-300"
                />
              </div>
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-gray-100 shrink-0">
              ...
              <button onClick={handleGuardarRegreso} disabled={saving || formRegreso.detalle.length === 0}
                ...
                {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Guardando...</> : "Confirmar Regreso"}
              </button>
            </div>
          </div>
        </div>
      ...
      {modalDetalle && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
              <h3 className="font-bold text-gray-800">Detalle — {modalDetalle.folio}</h3>
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-4 text-sm">
              ...
              {/* Productos */}
              {Array.isArray(modalDetalle.detalle) && modalDetalle.detalle.length > 0 && (
                <div>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {modalDetalle.detalle.map((item, i) => (
                          <tr key={i}>
                            <td className="px-3 py-2 font-medium text-gray-700">{item.producto_nombre || item.nombre}</td>
                            <td className="px-3 py-2 text-center">{item.cantidad_salida}</td>
                            <td className="px-3 py-2 text-center text-emerald-600 font-semibold">{item.cantidad_vendida ?? "—"}</td>
                            <td className="px-3 py-2 text-center text-amber-600">{item.cantidad_devuelta ?? "—"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {modalDetalle.estado === "cerrada" && (
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { label: "Efectivo esperado", value: fmt(modalDetalle.efectivo_esperado), bg: "bg-gray-50", text: "text-gray-700" },
                    { label: "Efectivo entregado", value: fmt(modalDetalle.efectivo_entregado), bg: "bg-emerald-50", text: "text-emerald-700" },
                    {
                      label: "Dif. efectivo",
                      value: fmt(modalDetalle.diferencia_efectivo),
                      bg: Math.abs(modalDetalle.diferencia_efectivo || 0) > 0.01 ? "bg-red-50" : "bg-green-50",
                      text: Math.abs(modalDetalle.diferencia_efectivo || 0) > 0.01 ? "text-red-700" : "text-green-700",
                    },
                    {
                      label: "Dif. garrafones",
                      value: modalDetalle.diferencia_garrafones ?? 0,
                      bg: Math.abs(modalDetalle.diferencia_garrafones || 0) > 0 ? "bg-amber-50" : "bg-green-50",
                      text: Math.abs(modalDetalle.diferencia_garrafones || 0) > 0 ? "text-amber-700" : "text-green-700",
                    },
                  ].map((s) => (
              ...
              {modalDetalle.observaciones && (
                <p className="text-xs text-gray-500 italic bg-gray-50 rounded-lg px-3 py-2">{modalDetalle.observaciones}</p>
              )}
              {modalDetalle.notas_regreso && (
                <p className="text-xs text-gray-500 italic bg-gray-50 rounded-lg px-3 py-2">{modalDetalle.notas_regreso}</p>
              )}
            </div>
          </div>
        </div>
