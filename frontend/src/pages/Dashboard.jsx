...
import {
  TrendingUp, Users, Package, Truck, ClipboardList,
  AlertTriangle, DollarSign, Droplets, ArrowUpRight,
  RefreshCw, Wallet, ShoppingBag, CheckCircle,
  BarChart2, Loader2
} from "lucide-react";
import {
...
const KPICard = ({ title, value, subtitle, icon: Icon, color, bg, to }) => {
  const inner = (
    <div className={`${bg} rounded-xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-all group`}>
      <div className="flex items-start justify-between mb-3">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{title}</p>
        <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${bg} border border-gray-200 group-hover:scale-110 transition-transform`}>
...
const Dashboard = () => {
  ...
  const nombreUsuario = (() => {
    try {
      ...
      return u.nombre || u.name || u.usuario || u.email?.split("@")[0] || "Admin";
    } catch {
      return "Admin";
    }
  })();
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              <KPICard
                ...
                icon={DollarSign} color="text-emerald-600" bg="bg-white"
                to="/ventas"
              />
              <KPICard
                title="Ticket Prom."
                ...
                subtitle="Este mes"
                icon={TrendingUp} color="text-sky-600" bg="bg-white"
                to="/reportes"
              />
              <KPICard
                title="Clientes"
                value={totalClientes}
                subtitle="Activos"
                icon={Users} color="text-violet-600" bg="bg-white"
                to="/clientes"
              />
              <KPICard
                title="Garrafones"
                value={resumen?.reparto?.garrafones_salida ?? statsReparto?.total_garrafones ?? 0}
                subtitle="Salieron este mes"
                icon={Package} color="text-blue-600" bg="bg-white"
                to="/inventario"
              />
              <KPICard
                title="Rutas Activas"
                ...
                subtitle="En camino"
                icon={Truck} color="text-orange-500" bg="bg-white"
                to="/reparto"
              />
              <KPICard
                title="Cartera"
                ...
                icon={Wallet} color="text-amber-600" bg="bg-white"
                to="/creditos"
              />
            </div>

            {/* ── Fila 2: gráfica + alertas ── */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">

              {/* Gráfica ventas del mes */}
                  ...
                  <ResponsiveContainer width="100%" height={210}>
                    <BarChart data={ventasDia} margin={{ top: 0, right: 10, left: 0, bottom: 0 }}>
                      ...
                      <YAxis tick={{ fontSize: 10, fill: "#9ca3af" }}
                        tickFormatter={v => `Q${(v/1000).toFixed(0)}k`} width={42} />
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                ...
                {alertasStock.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-sm text-gray-400">Stock en niveles normales</p>
                  </div>
                ) : (
                  <div className="space-y-2 max-h-52 overflow-y-auto">
                    {alertasStock.map((p, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-amber-50 rounded-xl border border-amber-100">
                        <div>
                          <p className="text-sm font-medium text-gray-700 truncate max-w-[130px]">{p.nombre}</p>
                          <p className="text-xs text-amber-600">Mín: {p.stock_minimo}</p>
                        </div>
                        <span className="text-xl font-bold text-red-600">{p.stock_actual}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                {statsReparto && (
                  <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-2 gap-2">
                    <div className="bg-sky-50 rounded-lg p-3 text-center">
                      ...
                      <p className="text-lg font-bold text-sky-600">{statsReparto.cerradas ?? 0}</p>
                    </div>
                    <div className="bg-emerald-50 rounded-lg p-3 text-center">
                      <p className="text-xs text-gray-500">Efectivo rep.</p>
                      <p className="text-sm font-bold text-emerald-600">{fmt(statsReparto.efectivo_total ?? 0)}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {resumen && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: "Total ventas mes",   value: fmt(resumen.ventas?.total_ventas),          color: "text-emerald-600", bg: "bg-emerald-50/60" },
                  { label: "Gasto en compras",   value: fmt(resumen.compras?.gasto_compras),         color: "text-red-600",     bg: "bg-red-50/60" },
                  { label: "Utilidad estimada",  value: fmt(resumen.utilidad_estimada),              color: resumen.utilidad_estimada >= 0 ? "text-emerald-700" : "text-red-600", bg: resumen.utilidad_estimada >= 0 ? "bg-emerald-50/60" : "bg-red-50/60" },
                  { label: "Cobrado créditos",   value: fmt(resumen.creditos?.cobrado_total),        color: "text-sky-600",     bg: "bg-sky-50/60" },
                ].map((s) => (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-100">
                        {["Folio", "Cliente", "Fecha", "Tipo", "Total", "Estado"].map(h => (
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {ultimasVentas.map((v) => (
                        <tr key={v.id} className={`hover:bg-gray-50 transition-colors ${v.estado === "cancelada" ? "opacity-50" : ""}`}>
                          ...
                          <td className="px-5 py-3">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${TIPO_VENTA[v.tipo_venta] || "bg-gray-100 text-gray-600"}`}>
                              {v.tipo_venta}
                            </span>
                          </td>
                          <td className="px-5 py-3 text-right font-bold text-gray-800">{fmt(v.total)}</td>
                          <td className="px-5 py-3 text-center">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${ESTADO_VENTA[v.estado] || ""}`}>
                              {v.estado}
                            </span>
                          </td>
                        </tr>
