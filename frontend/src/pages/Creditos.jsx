...
const BarraProgreso = ({ monto_total, saldo_actual }) => {
  ...
  const pct    = Math.min(100, Math.max(0, (pagado / parseFloat(monto_total)) * 100));
  return (
      ...
      <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
        <div
          className={`h-1.5 rounded-full transition-all duration-500 ${
            pct >= 100 ? "bg-emerald-500" : pct >= 50 ? "bg-sky-500" : "bg-amber-400"
          }`}
          style={{ width: `${pct}%` }}
        />
      </div>
...
const Creditos = () => {
  ...
  // Formularios
  const [formNuevo, setFormNuevo] = useState({ cliente_id: "", monto_total: "", venta_id: "" });
  ...
  const cargarCatalogos = async () => {
    const [resCl, resSt] = await Promise.allSettled([
      api.get("/clientes"),
      api.get("/creditos/estadisticas"),
    ]);
    if (resCl.status === "fulfilled") setClientes(toArray(resCl.value.data));
      ...
      <div className="space-y-5 p-1">
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Cartera total",    value: fmt(stats.cartera_total),    color: "text-gray-800",    icon: <CreditCard  className="w-4 h-4" /> },
              { label: "Saldo pendiente",  value: fmt(stats.saldo_pendiente),  color: "text-amber-600",   icon: <TrendingDown className="w-4 h-4" /> },
              { label: "Cobrado total",    value: fmt(stats.cobrado_total),    color: "text-emerald-600", icon: <TrendingUp   className="w-4 h-4" /> },
              { label: "Abonos este mes",  value: fmt(stats.abonos_mes),       color: "text-sky-600",     icon: <DollarSign   className="w-4 h-4" /> },
            ].map((s) => (
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-400 text-sm">No hay créditos registrados</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {["Cliente", "Fecha", "Monto total", "Progreso", "Último abono", "Estado", ""].map((h) => (
                      <th key={h} className={`px-5 py-3 text-xs font-semibold text-gray-500 uppercase ${
                        h === "Monto total" ? "text-right" :
                  ...
                  {creditosFiltrados.map((c) => (
                    <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3 min-w-[160px]">
                        <BarraProgreso monto_total={c.monto_total} saldo_actual={c.saldo_actual} />
                      </td>
                      <td className="px-5 py-3 text-gray-400 text-xs text-center">
                        {c.ultimo_abono || "—"}
                      </td>
                      <td className="px-5 py-3 text-center">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${ESTADO_STYLES[c.estado] || ""}`}>
                      ...
                      <td className="px-5 py-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {c.estado === "pendiente" && (
                            <button
                              onClick={() => { setModalAbono(c); setMontoAbono(""); setError(""); }}
      ...
      {modalAbono && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-800 text-lg flex items-center gap-2">
                <ArrowDownCircle className="w-5 h-5 text-emerald-500" /> Registrar Abono
              </h3>
              <button onClick={() => setModalAbono(null)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                <X className="w-5 h-5 text-gray-500" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-800">{modalAbono.cliente_nombre}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Monto total</span>
                  <span className="font-medium text-gray-800">{fmt(modalAbono.monto_total)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Saldo pendiente</span>
                  <span className="font-bold text-amber-600">{fmt(modalAbono.saldo_actual)}</span>
                </div>
                <BarraProgreso monto_total={modalAbono.monto_total} saldo_actual={modalAbono.saldo_actual} />
              </div>
              ...
              <div>
                ...
                {montoAbono && (
                  <p className={`text-xs mt-1 ${
                    ...
                    Saldo restante: {fmt(Math.max(0, parseFloat(modalAbono.saldo_actual) - (parseFloat(montoAbono) || 0)))}
                  </p>
                )}
              </div>
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-gray-100">
              ...
              <button
                ...
                disabled={saving || !montoAbono || parseFloat(montoAbono) <= 0 || parseFloat(montoAbono) > parseFloat(modalAbono.saldo_actual)}
                className="flex-1 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-sm font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-2"
              >
                {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Guardando...</> : "Registrar Abono"}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 p-6 space-y-4">
                ...
                {[
                  ...
                  <div key={label}>
                    ...
                    <p className="font-medium text-gray-800 mt-0.5">
                      {label === "Estado"
                        ? <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${ESTADO_STYLES[val] || ""}`}>
                            {ESTADO_ICON[val]} {val}
                          </span>
                        : val}
                    </p>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: "Monto total",  value: fmt(modalDetalle.monto_total),  bg: "bg-gray-50",     text: "text-gray-800" },
                  { label: "Saldo pendiente", value: fmt(modalDetalle.saldo_actual), bg: "bg-amber-50",  text: "text-amber-700" },
                  { label: "Pagado",       value: fmt(parseFloat(modalDetalle.monto_total) - parseFloat(modalDetalle.saldo_actual)), bg: "bg-emerald-50", text: "text-emerald-700" },
                ].map((s) => (
              ...
              <BarraProgreso monto_total={modalDetalle.monto_total} saldo_actual={modalDetalle.saldo_actual} />
              ...
              {Array.isArray(modalDetalle.abonos) && (
                <div>
                  ...
                  {modalDetalle.abonos.length === 0 ? (
                    <p className="text-xs text-gray-400 italic text-center py-4">Sin abonos registrados</p>
                  ) : (
                    <div className="space-y-1.5 max-h-48 overflow-y-auto">
                      {modalDetalle.abonos.map((a) => (
                        <div key={a.id} className="flex justify-between items-center bg-gray-50 px-3 py-2 rounded-lg text-sm">
                          <div className="flex items-center gap-2 text-gray-500">
                            <CheckCircle className="w-3.5 h-3.5 text-emerald-500" />
                            <span className="text-xs">{a.created_at}</span>
                          </div>
                          <span className="font-bold text-emerald-600">{fmt(a.monto)}</span>
                        </div>
                      ))}
                    </div>
                  )}
