...
const Ventas = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setVentas(vRes.data);
      setProductos(
        pRes.data.filter((p) => p.activo === true || p.activo === "true"),
      );
  ...
  const guardarVenta = async () => {
    ...
    if (form.metodo_pago === "credito" && !form.cliente_id) {
      showToast("Selecciona un cliente para ventas a crédito", "error");
      return;
    }
    try {
      setSaving(true);
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((v) => (
                  <tr
                    key={v.id}
                    className={`border-t border-gray-50 hover:bg-gray-50/70 transition ${v.estado === "cancelada" ? "opacity-60" : ""}`}
                  >
                    ...
                    <td className="px-5 py-3">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-medium ${TIPO_VENTA_COLORS[v.tipo_venta] || "bg-gray-100 text-gray-600"}`}
                      >
                        {v.tipo_venta}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-3 text-center">
                      <span
                        className={`px-2.5 py-1 rounded-full text-xs font-medium ${ESTADO_COLORS[v.estado] || ""}`}
                      >
                        {v.estado}
                      </span>
                    </td>
      ...
      p.stock_actual <= 0
        ...
        : "hover:bg-white hover:shadow-sm border-transparent hover:border-blue-100"
                      ...
                      >
                        ...
                        <p
                          className={`text-xs mt-0.5 ${p.stock_actual <= 0 ? "text-red-400 font-medium" : "text-gray-400"}`}
                        >
                          {p.stock_actual <= 0
                            ? "Sin stock"
                    ...
                    {productos.length === 0 && (
                      <p className="col-span-3 text-center py-4 text-gray-400 text-xs">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <button
                        onClick={() =>
                          setForm({
                            ...form,
                            garrafones_recibidos: Math.max(
                              0,
                              form.garrafones_recibidos - 1,
                            ),
                          })
                        }
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setForm({
                            ...form,
                            garrafones_recibidos: parseInt(e.target.value) || 0,
                          })
                        }
                      ...
                      <button
                        onClick={() =>
                          setForm({
                            ...form,
                            garrafones_recibidos: form.garrafones_recibidos + 1,
                          })
                        }
                        className="w-9 h-9 rounded-xl bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition font-bold text-gray-600"
                      >
                  ...
                  <div>
                    ...
                    {form.metodo_pago === "credito" && (
                      <p className="text-xs text-amber-600 mt-1">
                        ⚠ Crédito → se marca pendiente automáticamente
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {[
                    ...
                    [
                      "Cliente",
                      detailModal.cliente_nombre || "Público General",
                    ],
                    ...
                    [
                      "Estado",
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${ESTADO_COLORS[detailModal.estado]}`}
                      >
                        {detailModal.estado}
                      </span>,
                    ],
                    ...
                    <div key={i}>
                      ...
                      <div className="font-medium text-gray-800 capitalize mt-0.5">
                        {val}
                      </div>
                    </div>
                  ))}
                  ...
                  {!detailData ? (
                    ...
                    <div className="space-y-1">
                      {detailData.items.map((item, i) => (
                        <div
                          key={i}
                          className="flex justify-between items-center text-sm bg-gray-50 px-3 py-2.5 rounded-xl"
                        >
                          <div>
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold text-blue-700">
                        {detailModal.garrafones_recibidos}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="border-t border-gray-100 pt-4 space-y-1.5 text-sm">
                  <div className="flex justify-between text-gray-500">
                    <span>Subtotal</span>
                    <span>{fmt(detailModal.subtotal)}</span>
                  </div>
                  {Number(detailModal.descuento) > 0 && (
                    <div className="flex justify-between text-gray-500">
                      <span>Descuento</span>
                      <span>- {fmt(detailModal.descuento)}</span>
                    </div>
                  )}
                ...
                {detailModal.estado === "cancelada" && (
                  <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3 text-center">
                    ...
                    <p className="text-red-400 text-xs mt-0.5">
                      El stock fue restaurado al momento de la cancelación
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
              <div className="p-6 text-center space-y-3">
                ...
                <p className="text-sm text-gray-500">
                  Los productos se regresarán al inventario automáticamente.
